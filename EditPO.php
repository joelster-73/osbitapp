...
<head>
	...
	<title>Purchase Orders | Edit</title> <!--Appears in the tab-->
...
<?php
	...
	if (isset($_GET['formSubmitted'])) {
		...
		if ($exists) { //if the record has been found
			echo "<script type='text/javascript'>window.location.href='EditPOForm.php?val='+".$_GET['POID'].";</script>"; //redirects user to form to edit if supplier found
		} else {
			echo "<script type='text/javascript'>window.alert('This Purchase Order was not found'); window.location.href='EditPO.php';</script>"; //informed supplier not found if appropiate
		}
	}
?>

<body onload="addNavBar('navigation',window.location.href); formStyling('formTable'); calcTotalCost(); formatMoney(); formatPercent();" onchange="tableStyling('formTable','tableStyleTag');"><!--calls function when body loads-->
  <div id="navigation"></div>
	...
	<div class="content">
		...
		<form id="miniForm" method="get">
			...
			<h3>Find a Purchase Order to Edit</h3> <br> <!--Title of the form-->
			...
			<input type="submit" value="Find Purchase Order"><!--To retrieve data to be editted-->
